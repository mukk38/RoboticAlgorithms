...
public class AcisalHizDialogPage extends TabbedPaneOrtakDialogPage {

	

	public AcisalHizDialogPage(String name, int indeks) {
		super(name, indeks);
	}
	...
	public JPanel getGrafikPanel(int i) {
		...
		dynamicWindowApproach = new ChartCategory<String>("Dynamic Window Approach",
				dynamicWindowApproachFieldHighlight);
		CategoryRange<String> colors = new CategoryRange<String>().add(aStar).add(Lee).add(potentialField).add(rrt)
				.add(rrtStar).add(dynamicWindowApproach);
	...
	private DefaultChartModel chartModelOlustur() {
		...
		if (Modeller.leeRobotModel.get(robotIndeks).getAlgoritmaModel().size()>algoritmaIndeks) {
			leeDeger = Modeller.leeRobotModel.get(robotIndeks).getAlgoritmaModel().get(algoritmaIndeks).getAcisalHiz();
		}
		if (Modeller.potentialFieldRobotModel.get(robotIndeks).getAlgoritmaModel().size()>algoritmaIndeks) {
			potentialFieldDeger = Modeller.potentialFieldRobotModel.get(robotIndeks).getAlgoritmaModel()
					.get(algoritmaIndeks).getAcisalHiz();
		}
		if (Modeller.rrtRobotModel.get(robotIndeks).getAlgoritmaModel().size()>algoritmaIndeks) {
			rrtDeger = Modeller.rrtRobotModel.get(robotIndeks).getAlgoritmaModel().get(algoritmaIndeks).getAcisalHiz();
		}
		...
		DefaultChartModel chartModel = new DefaultChartModel("Açýsal Hýz Grafik");
