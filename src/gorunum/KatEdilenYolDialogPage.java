...
public class KatEdilenYolDialogPage extends TabbedPaneOrtakDialogPage {


	public KatEdilenYolDialogPage(String name,int indeks) {
		super(name,indeks);
	}
	...
	public JPanel getGrafikPanel(int i) {
        ...
        barChart.setTitle("Kat Edilen Yol");
	...
	private DefaultChartModel chartModelOlustur() {
		...
		if (Modeller.leeRobotModel.get(robotIndeks).getAlgoritmaModel().size()>algoritmaIndeks) {
			leeDeger = Modeller.leeRobotModel.get(robotIndeks).getAlgoritmaModel().get(algoritmaIndeks).getKatEdilenYol();
		}
		if (Modeller.potentialFieldRobotModel.get(robotIndeks).getAlgoritmaModel().size()>algoritmaIndeks) {
			potentialFieldDeger = Modeller.potentialFieldRobotModel.get(robotIndeks).getAlgoritmaModel()
					.get(algoritmaIndeks).getKatEdilenYol();
		}
		if (Modeller.rrtRobotModel.get(robotIndeks).getAlgoritmaModel().size()>algoritmaIndeks) {
			rrtDeger = Double.parseDouble(Modeller.rrtRobotModel.get(robotIndeks).getAlgoritmaModel().get(algoritmaIndeks).getRrtDeger().split("")[0]);
		}
		if (Modeller.rrtStarRobotModel.get(robotIndeks).getAlgoritmaModel().size()>algoritmaIndeks) {
			rrtStarDeger = Double.parseDouble(Modeller.rrtStarRobotModel.get(robotIndeks).getAlgoritmaModel().get(algoritmaIndeks)
					.getRrtDeger().split("")[0]);
		}
		...
		DefaultChartModel chartModel = new DefaultChartModel("Kat Edilen Yol Grafik");
